...
Função processar_folder_link:

Agora verifica se o nome da pasta contém "LINK PARCEIROS" e define a cor como vermelha (color = "red") para essas pastas.

Mantém a cor azul como padrão para outras pastas LINK.

Adição de LineString ao Mapa:

As LineString são adicionadas ao mapa com as cores e estilos corretos. Se a pasta for "LINK PARCEIROS", a cor será vermelha. Caso contrário, a cor será definida pelo estilo do KML ou será azul por padrão.

Se a linha estiver em "EM ANDAMENTO", ela será tracejada (dash_array = "5, 5"). Caso contrário, será sólida.

Exibição no Streamlit:

O mapa é exibido com as LineString coloridas corretamente.

As tabelas são exibidas com as informações de distância e status das rotas.

Como Testar
Carregue um arquivo KML que contenha pastas "LINK PARCEIROS".

Verifique se as LineString correspondentes são exibidas no mapa em vermelho.

Confira se as linhas em "EM ANDAMENTO" são tracejadas e as concluídas são sólidas.

Verifique as tabelas para garantir que as informações de distância e status estejam corretas.

Se tudo estiver funcionando corretamente, as linhas de "LINK PARCEIROS" devem aparecer em vermelho no mapa, e as informações devem ser exibidas corretamente nas tabelas.

está com problema de exibição da linha vermelha quando estiver informação, dentro de alguma pasta que conter LINK PARCEIROS , porém , nem na tabela ta reconhecendo , o codigo deve está apresentando alguma inconsistencia
Vamos revisar o código para garantir que a funcionalidade de "LINK PARCEIROS" seja corretamente implementada e que as linhas vermelhas sejam exibidas no mapa, além de garantir que as informações sejam reconhecidas nas tabelas.

Problemas Identificados
Verificação de "LINK PARCEIROS":

A função processar_folder_link deve garantir que todas as pastas que contêm "LINK PARCEIROS" no nome sejam corretamente identificadas.

A cor vermelha deve ser aplicada a todas as LineString dentro dessas pastas.

Exibição no Mapa:

As LineString devem ser adicionadas ao mapa com a cor correta (vermelho para "LINK PARCEIROS").

O estilo tracejado deve ser aplicado apenas para "EM ANDAMENTO".

Tabelas:

As informações de "LINK PARCEIROS" devem ser corretamente registradas nas tabelas.

Código Corrigido
Aqui está o código corrigido para garantir que a funcionalidade de "LINK PARCEIROS" funcione corretamente:
